...
function Projects({ projects }: ProjectsProps) {
  ...
  const [filteredProjects, setFilteredProjects] = useState<Project[]>([]); // Initialize with an empty array of Projects
  ...
  // Define applyFilters function - MUST be BEFORE useState initialization
  ...
  // Memoize filteredProjects - useMemo is from 'react'
  useEffect(() => {
...
const Card = ({ title, image, description }: Project) => {
  ...
  const getFirstLine = (desc: string) => {
    ...
    return firstSentence ? firstSentence.trim() : desc.trim();
  };

  const firstLineDescription = getFirstLine(description);
  ...
  return (
    ...
    >
      ...
      <div className="md:py-4 relative flex flex-col">
        <motion.div
        ...
        <div className="overflow-hidden max-md:hidden mb-7">
          <motion.p
            initial={{ y: 0, opacity: 0 }}
            animate={{ y: hover ? -10 : 0, opacity: hover ? 1 : 0 }}
            className="absolute text-white/50 text-sm md:text-base"
          >
      ...
      <motion.div
        layout
        className="aspect-square overflow-hidden rounded-xl md:rounded-3xl"
      ...
      >
        <motion.img
          ...
          height={400}
          alt={title}
          className="object-cover h-full w-full object-center transform scale-100 hover:scale-110 transition-transform duration-300 mt-2"
        />
      </motion.div>
    </motion.div>
  );
