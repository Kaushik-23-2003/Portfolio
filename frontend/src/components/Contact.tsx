...
const Contact = ({ email, social_handle, about }: ContactProps) => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/contact', { // Adjust the path if your API is hosted differently
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        setStatus("DONE");
        setStatusText(data.message || "Message sent successfully!"); // Use message from backend if available
        ...
        setStatus("ERROR");
        setStatusText(data.message || "Error in sending message. Please try again."); // Use message from backend if available
  ...
  return (
    <motion.section className="relative" id="contact">
      ...
      {/* <footer className="flex items-center justify-between md:px-8 px-2 py-4 text-sm">
        <Transition>
          <div>© {new Date().getFullYear()} Hariharan Kaushik</div>
        </Transition>
        <Transition>
          <p>
            developed by @
